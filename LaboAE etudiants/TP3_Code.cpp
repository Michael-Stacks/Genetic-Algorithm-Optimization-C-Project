...
TSolution Croisement(TSolution Parent1, TSolution Parent2, TProblem unProb, TAlgo &unAlgo)
{
	...
	int point_croisement = rand() % unProb.N;

	// Copier la première partie de Parent1
	for (int i = 0; i < point_croisement; ++i)
	{
		Enfant.Selec[i] = Parent1.Selec[i];
	}

	// Copier la seconde partie de Parent2
	for (int i = point_croisement; i < unProb.N; ++i)
	{
		Enfant.Selec[i] = Parent2.Selec[i];
	}

	EvaluerSolution(Enfant, unProb, unAlgo);  // Évaluation de l'enfant
	return Enfant;
}
...
void Remplacement(std::vector<TSolution> &Parents, std::vector<TSolution> Enfants, TProblem unProb, TAlgo unAlgo)
{
	// Fusionner les parents et les enfants dans une nouvelle population
	std::vector<TSolution> NouvellePop = Parents;
	NouvellePop.insert(NouvellePop.end(), Enfants.begin(), Enfants.end());

	// Trier la population par fonction objectif (en maximisant)
	TrierPopulation(NouvellePop, 0, NouvellePop.size());

	// Garder les meilleures solutions
	Parents.clear();
	for (int i = 0; i < unAlgo.TaillePop; ++i)
	{
		Parents.push_back(NouvellePop[i]);
	}
}
